...
list.files(pattern = "*\\d.txt")
mydata <- do.call("rbind", sapply(list.files(pattern = "*\\d.txt"), readplate, 17, platemap, simplify = F))  %>%
  separate(filename, c("date", "line1", "line2", "line3", "replicate"), sep = "_") %>%
  dplyr::select(-date) %>%
  mutate(line = "NA",
         line = if_else(col %in% c(1:4), line1, line), 
         line = if_else(col %in% 5:8, line2, line), 
         line = if_else(col %in% 9:12, line3, line)) %>%
  mutate(replicate = substr(replicate, 1,1)) %>%
  dplyr::select(-line1, -line2, -line3)
...
mydata.norm <- 
  ...
  dplyr::filter(grepl("C04|Parental", line)) %>%
  arrange(desc(line))%>%
  left_join(line.lut)
...
mydata.norm %>%
  dplyr::filter(conc != 0) %>%
  ggplot(aes(x=conc,  y = abs.norm, col = line.short)) +
  geom_point() + 
  geom_smooth() + 
...
mypal <- palette.colors(3)
par(mfrow = c(2,2), mar = c(4.1, 2.6, 1.1, 1.1))


myylim <- c(0,1.3)
myyt <- seq(myylim[1], myylim[2], by = 0.4)
myytlab <- myyt
